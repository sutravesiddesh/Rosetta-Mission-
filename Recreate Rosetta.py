...
def lambert_solver(r1, r2, dt, mu):
    r1 = np.array(r1, dtype=float).flatten()
    r2 = np.array(r2, dtype=float).flatten()
    ...
    sin_theta = np.linalg.norm(cross) / (r1_norm * r2_norm)

    theta = np.arctan2(sin_theta, cos_theta)

    A = sin_theta * np.sqrt(r1_norm * r2_norm / (1 - cos_theta))
    ...
    for _ in range(max_iter):
        ...
        y = r1_norm + r2_norm + A * (z * S - 1) / np.sqrt(max(C, 1e-8))
        if y < 0:
            z += 0.1
            continue
